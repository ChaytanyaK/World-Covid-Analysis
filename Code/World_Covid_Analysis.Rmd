...
```{r}
thirty_countries_covid <- read_csv("World_Covid_Data.csv")
```
...
```{r}
thirty_countries_covid %>% 
  dplyr::select(location, people_fully_vaccinated, people_vaccinated, new_deaths) %>% 
  group_by(location) %>% 
  drop_na(new_deaths) %>% 
...
```{r}
thirty_countries_covid %>% 
  filter(date == "2021-03-08") -> mydata3 ## on date == "2021-04-24"
```
...
one_day_thirty_countries[-c(18),]->t1
view(t1)

t1
eq1=lm(total_deaths_per_million~smoker_total+extreme_poverty+gdp_per_capita+total_vaccinations+stringency_index+population_density,data=t1)
summary(eq1)

t1[-c(28),]->t2
t2

eq2=lm(total_deaths_per_million~smoker_total+extreme_poverty+gdp_per_capita+total_vaccinations+stringency_index+population_density,data=t2)
summary(eq2)

xx2<-lm(total_deaths_per_million~1,data=t2)
xx2
x3<-step(xx2, direction = "forward",scope=formula(eq1))
x3
summary(x3)

t1 %>% 
  ggplot(aes(x = population_density, y = total_deaths_per_million)) +
  geom_point() +
  geom_text_repel(aes(label = location)) +
  geom_smooth(se = FALSE) +
  labs(title = "Death per million vs Extreme_Poverty") +
  xlab("Poverty") +
  ylab("Total Death by COVID per Million")


lm(total_deaths_per_million ~ gdp_per_capita, data = mydata3) -> p
summary(p)

peq1=lm(total_deaths_per_million~smoker_total+extreme_poverty+gdp_per_capita+total_vaccinations+stringency_index,data=mydata3)
summary(peq1)
...
peq1=lm(total_deaths_per_million~smoker_total+extreme_poverty+gdp_per_capita+total_vaccinations+stringency_index+population_density,data=mydata3)
summary(peq1)
vif(peq1)
#-----Malta--------
t1
...
#STEPWISE FORWARD Analysis

xx2<-lm(total_deaths_per_million~1,data=mydata3)
xx2
x3<-step(xx2, direction = "forward",scope=formula(peq1))
x3
summary(x3)

#-----Remove Malta-----
xx2<-lm(total_deaths_per_million~1,data=t1)
xx2
x3<-step(xx2, direction = "forward",scope=formula(eq1))
x3
summary(x3)
...
peq1=lm(new_deaths~gdp_per_capita+population_density+gdp_per_capita*population_density,data=mydata3)
peq1
summary(peq1)
